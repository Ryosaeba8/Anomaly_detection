...
from repen_network import REPEN
...
parser.add_argument("--runs", type=int, default=4, help="how many times we repeat the experiments to obtain the average performance")
...
def train_and_save(args, data) :
    ...
    print("Mode :", args.mode, "Total outliers :",
           args.known_outliers, "Real outliers", real_ano)
    for i in range(args.runs) :
        x_train, y_train = get_train(data, args)
        repen = REPEN(**args.__dict__)
        repen.fit(x_train, y_train)
        scores = repen.decision_function(data.X_val)
...
for mode in ["unsupervised", "semi_supervised", "supervised"] :
    args.mode = mode   
